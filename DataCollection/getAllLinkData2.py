            ...
            if entry.ip:
                ...
                try:
                  if idList.count(ALL_Faces_ids[entry.ip]) != 0:
                    ...
                    if ALL_Faces_ids[entry.ip] == 5 or ALL_Faces_ids[entry.ip] == 6:
                      curlCmd = '/usr/bin/curl -s -L http://128.252.153.27/bw/' + repr(ALL_Faces_ids[entry.ip]) + '/' + repr(current_time) + '/' + entry.rxdata() + '/' +entry.txdata()
                    else:
