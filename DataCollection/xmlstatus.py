...
class BWData:    
    ...
    def getrxdata(self):
        rtn = repr(self.rxdata['sz'])
        return rtn

    def gettxdata(self):
        rtn = repr(self.txdata['sz'])
        return rtn
    ...
    def printout(self):
        ...
        for entry in self.face_list:
            if entry.ip:
                #print '    ' + entry.str()
                try:
                  if ALL_Faces_ids[entry.ip]:
                    # ALL_Faces_id 15 is for the WU to KANS link
                    curlCmd = 'curl -L http://128.252.153.27/bw/' + repr(ALL_Faces_ids[entry.ip]) + '/' + repr(current_time) + '/' + entry.txdata() + '/' +entry.rxdata()
